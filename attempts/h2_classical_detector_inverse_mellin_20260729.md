# H2 Classical Detector Inverse Mellin Attempts
...
Can Mathlib's Fourier-derived Mellin inversion theorem discharge the exact vertical-line identity
for the project's actual smoothed truncated-Mobius detector, including Gamma integrability and
the infinite sum-integral exchange?

## Attempt ledger

| round | mode | observation | decision |
| --- | --- | --- | --- |
| 1 | `ROUTE_SELECTION` | H10's next genuine source edge is a broad curve-intersection/Riemann--Roch producer, while H11 still lacks a source-backed sparse-exception amplifier. H2 has one exact named missing proposition immediately upstream of the global contour shift. | Select `ClassicalDetectorInverseMellinLine` as a materially distinct historical mechanism. |
