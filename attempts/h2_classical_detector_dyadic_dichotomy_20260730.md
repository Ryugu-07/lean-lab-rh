# H2 Classical Detector Dyadic Dichotomy Attempts
...
Can the actual Maynard--Pratt smoothed detector at an actual zeta zero be truncated and
partitioned into powers-of-two blocks with a fully explicit simultaneous error budget, so that
Lean derives the source Type-I/Type-II disjunction rather than only an abstract finite
pigeonhole theorem?
...
| --- | --- | --- | --- |
| 1 | `PARENT_CLOSURE` | H7 Fourier-topology receipt `2408208cbadbf7ba1c5bfe1dae28849a429627fc` passed run `30488165861`, job `90698985577`, in `1m35s`. | Stop only H7's local topology campaign and rerank all historical families. |
| 2 | `USER_POLICY_ALIGNMENT` | Historical work is intended to search for overlooked strength or missing combinations in human routes. Exhaustion is not required before opening a conjecture, falsification, or direct proof attempt. | Treat source reconstruction as omission-seeking evidence, not as a prerequisite that closes discovery. |
| 3 | `CROSS_FAMILY_SELECTION` | H1's next source edge needs new nonstationary oscillatory analysis. H12's next edge combines a global indented argument principle, top variation, and signed base orientation. H2 Lemma 23 is one fixed finite paragraph attached to an already compiled actual contour shift. | Select H2 without relying on adjacency. |
...
- `compiled_left_context`: actual truncated-Mobius coefficients and coefficient gap; actual
