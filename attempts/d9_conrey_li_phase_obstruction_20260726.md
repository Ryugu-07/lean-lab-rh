# D9 Conrey--Li Phase Obstruction
...
- `exact_target`: kernel-check the dense-phase and coordinate logic in the nonnumerical Sarnak
  obstruction to the Conrey--Li shifted positivity proposal.
- `relation_to_RH`: the failed positivity would have implied a zero-free right half-strip and
  hence RH-type zero location. Falsifying it closes only that proposed sufficient condition.
- `success`: generic phase crossing, exact reciprocal shift algebra, and an actual-xi
  specialization with value-distribution premises visible.
- `falsification`: an exact countermodel to the topology claim or a corrected source identity.

## Attempt log

| phase | action | result | next decision |
| --- | --- | --- | --- |
| `PARENT_PUBLIC_CLOSURE` | Closed the H12 critical-indentation campaign. | Final ledger `4ee3b000bf2baff304edfedc215cf9143b399cea` passed run `30195238711`, job `89775469902`, in `1m53s`. | Return to cross-family route selection. |
...
- `compaction_state`: resumed from a compacted live state; re-read governance, HANDOFF, hard-gap
  DAG, route atlas, H1/H10 cards, D9 records, current proof surfaces, and the primary Conrey--Li
