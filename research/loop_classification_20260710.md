...
| M1-14 | HARD_GAP_REDUCED | remove truncated-Perron subedge | Compiled the exact absolute `C*(N+1)^2/T` Mobius truncated Perron formula, including residue, dominated series-integral exchange, and half-integral error summation. |
| M1-15 | HARD_GAP_REDUCED | remove contour-balancing subedge; close RH-specialized F1 | Compiled the RH-specialized Balazard-Saias estimate and connected it to the Burnol transformed-error consumer without an `hBS` premise. |
| M1-16 | HARD_GAP_REDUCED | remove aligned reverse-criterion edge | Compiled `baezDuarteComplexTargetL2 ∈ baezDuarteComplexKernelClosure -> Mathlib.RiemannHypothesis` by the source Mellin zero obstruction and exact full-line tail structure. |
