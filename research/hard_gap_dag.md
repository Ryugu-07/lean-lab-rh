...
| H6-Q1 | open; selected; Loop 11 partial public implementation | Kernel-check the initial, final, and barrier certificates for the second row of Polymath Table 1 and derive `deBruijnNewmanAllZerosReal (1/5)` without hypotheses. | The conditional Polymath consumer, finite-height-RH initial bridge, complete Theorem 1.3 normalization, explicit final-region consumer, equation `(htz)`, fixed principal-power `5*pi/4` line integrability, every positive-integer local residue normalization, the exact adjacent and finite `R_(0,0)` shifts, full Titchmarsh `(xio)`, finite equation `(39)`, and both actual-source contour shifts are public K0. Loop 10 additionally compiles the upper-half-plane alpha derivative, `Im(alpha_n)>=-0.15`, same-half-plane transport, exact second-order `log M_0` Taylor bound, displacement and main-term identities, source Gaussian equation `(ax)`, exact prefactor/Stirling ratio, and the complete conditional conversion from a Boyd `R_2` bound through constants `0.0205`, `0.246`, and `0.33`; implementation/evidence CI runs `29657235672` and `29657362457` passed. Proposition 6.1 is still open. Loop 11 compiles the project-normalized scaled-Gamma continuation, imaginary-axis boundary, right-half-plane analytic reduction, Phragmen--Lindelof application interface, mirror-ray/Stokes transfer, denominator bounds, and final `<41/2000` comparison; implementation commit `3409b7175eb24f1e0f01377795334a08e5f80384` passed public CI run `29659089400`. The actual Boyd/Nemes equation `(15)` remainder representation, contour rotation, and Stieltjes growth inputs remain open. Proposition 6.3 also needs the absent Arias de Reyna Proposition 6.2 expansion. Strict finite-sum certificates, finite RH through `3*10^12`, compact barrier winding, H6-E/G8, and RH remain open. External Arb output is navigation evidence only. |
