...
| H6-Q1 | open; selected; Loop 10 preregistered | Kernel-check the initial, final, and barrier certificates for the second row of Polymath Table 1 and derive `deBruijnNewmanAllZerosReal (1/5)` without hypotheses. | The conditional Polymath consumer, finite-height-RH initial bridge, complete Theorem 1.3 normalization, explicit final-region consumer, equation `(htz)`, fixed principal-power `5*pi/4` line integrability, every positive-integer local residue normalization, the exact adjacent and finite `R_(0,0)` shifts, the full Titchmarsh `(xio)` identity, finite equation `(39)`, and both actual-source contour shifts `(rtn-def)`/`(RTN-def)` are public K0. Loop 10 selects the exact Proposition 6.1 positive-term estimate before Proposition 6.3. Its first open obligation is Boyd's effective complex Stirling remainder, source Lemma 5.1(v), followed by the upper-half-plane alpha/Taylor and Gaussian constant chain. Proposition 6.3 additionally needs the absent Arias de Reyna Proposition 6.2 expansion. Strict finite-sum certificates, the finite RH premise through `3*10^12`, compact barrier winding, H6-E/G8, and RH remain open. External Arb output is navigation evidence only. |
