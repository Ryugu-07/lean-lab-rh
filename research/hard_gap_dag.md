...
| G8 | H6-E | open | Prove `Lambda <= 0`, equivalently all zeros of the compiled source-normalized `H_0` are real. The local pair law and adjacent estimate `(gap^2)'<=8` compile, but the exact quadratic audit proves that generic estimate is sharp and cannot yield a fixed backward interval. The missing edge is theta-specific height-aware continuation through time zero, including the first possible repeated zero. |
