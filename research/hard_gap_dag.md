...
| OBS-H6-XI-LOGCONCAVITY-LEAN-01 | H6-E/G8 | complete local source obstruction | At external commit `7a89db1`, the advertised log-concavity predicate and full-kernel endpoints conclude only `True`; decisive perturbation claims are custom axioms and the numerical script omits the infinite tail. The exact no-convergence Hurwitz axiom is Lean-refuted by `F_n=1` and `G(z)=z-i`. Reject only this external certification chain: actual Xi-kernel TP2, TP-infinity, H6-E/G8, and RH remain open. |
