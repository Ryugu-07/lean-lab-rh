...
| H6-H | open; H2b locally audited | Formalize the all-real-zero predicate, de Bruijn forward preservation, threshold existence, and threshold closedness for the exact H6-B/H6-H1 family. | The exact time-zero all-real-zero predicate, bijective zero coordinate, strict strip, and RH equivalence are public. Campaign `CAMPAIGN-20260717-H6-THRESHOLD-CLOSEDNESS-01` locally compiles and fully audits exact closedness through joint continuity, nonvanishing, isolated zeros, and Jensen persistence; public implementation CI is pending. Forward preservation, threshold nonemptiness/upper-ray structure, and the global heat-flow zero theory remain open. |
