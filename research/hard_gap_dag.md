...
| H6-Q1 | open; selected; Loop 6 finite decomposition public | Kernel-check the initial, final, and barrier certificates for the second row of Polymath Table 1 and derive `deBruijnNewmanAllZerosReal (1/5)` without hypotheses. | The conditional Polymath consumer, finite-height-RH initial bridge, complete Theorem 1.3 normalization, explicit final-region consumer, equation `(htz)`, fixed principal-power `5*pi/4` line integrability, every positive-integer local residue normalization, the exact adjacent infinite shift `I_N(s)=(N+1)^(-s)+I_(N+1)(s)`, and the finite `I_0`/`R_(0,0)` decompositions are public K0. Loop 5's upper finite interval has a fixed stagger, explicitly recorded in attempts. Loop 6 implementation `7ea4238b` passed public CI run `29637745080`. Titchmarsh `(xio)`, effective approximation, strict finite-sum certificate, finite RH premise through `3*10^12`, and compact barrier winding certificate remain open. External Arb output is navigation evidence only. |
