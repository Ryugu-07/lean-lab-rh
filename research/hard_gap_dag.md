...
| H6-H | open; H2a-H2c complete; H2d selected | Formalize the all-real-zero predicate, de Bruijn forward preservation, threshold existence, and threshold closedness for the exact H6-B/H6-H1 family. | The exact zero coordinate, strict time-zero strip, RH equivalence, closedness, and arbitrary-two-time forward preservation are public. Campaign `CAMPAIGN-20260717-H6-UPPER-HALF-01` fixes the quantitative strip contraction `Im(z)^2 <= 1-2*t` on `0<=t<=1/2` and the unconditional good-time witness at `t=1/2`. A finite real threshold, H6-E/G8, and the remaining global heat-flow zero theory stay open. |
