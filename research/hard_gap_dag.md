...
| W1c1c0 | complete | `WeilCompactLaplaceZeroCutoff.lean` proves the selected-height xi zero-side limit for the reflection symmetrization of every smooth compactly supported additive-log function. Lean derives transform differentiability, exact reflection, complete divisor summability, arbitrary integration by parts, inverse-sixth-power fixed-strip decay, and top-edge vanishing from smoothness and compact support alone. Local exact targets, standard-only axiom audit, scans, and the 8,680-job build pass. The compact arithmetic evaluation, W2/G7, and RH remain open; public implementation CI is pending. |
