...
| H6-H | open globally; H2a-H2d and zero-dynamics interfaces compiled | Formalize the all-real-zero predicate, de Bruijn forward preservation, threshold existence, threshold closedness, and the simple-zero dynamics interface for the exact H6-B/H6-H1 family. | The framework reaches the unconditional `t=1/2` good witness, unique real simple paths, pair-removed dynamics, and the adjacent bound `(gap^2)'<=8`. The bound is sharp in the generic heat class. Global enumeration/continuation and repeated-zero treatment remain open. |
| H6-X | complete first-two endpoint | Prove theta-specific Li information for the exact heat family beyond the generic heat PDE. | `DeBruijnNewmanLiMoments.lean` proves for every real `t` the exact values `F_t(1)=8A`, `F_t'(1)=16B`, `F_t''(1)=32C`, the positive-weight bound `B^2<=A*C`, and strict real positivity of the first two Li differential expressions. Implementation `2bc304e9fe2473519c398269b26b0b06b715e593` passed public CI run `29541314279`. This is finite-index route infrastructure with `hard_gap_delta=0`; no all-index extrapolation is made. |
| H6-E | open | Prove all zeros of `H_0` are real, equivalently `Lambda <= 0` in the audited normalization. | The generic adjacent-gap attack is exhausted by `OBS-H6-ADJACENT-GAP-EIGHT-01`, and generic positive-kernel/Hankel extrapolation beyond the first two Li values is eliminated by `OBS-H6-POSITIVE-COSH-LI3-01`. A new attack must use quantitative theta-specific shape, height-aware continuation, or a different all-index invariant. The endpoint is unchanged. |
...
| OBS-H6-POSITIVE-COSH-LI3-01 | H6-X/H6-E | public implementation verified; evidence CI pending | A normalized positive two-atom `cosh` transform is entire, reflection symmetric, and has positive real first and second standard Li values, but its third Li value is strictly negative. Implementation commit `5fdfc5c7437349735c57552a75838f16b4d63f5e` passed public CI run `29543145545`, job `87769424525`. Positive-kernel and ordinary moment/Hankel positivity therefore cannot generically scale H6-Z to all indices; quantitative theta-kernel structure remains necessary. |
