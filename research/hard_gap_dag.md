...
| H6-H | open globally; H2a-H2e and zero-dynamics interfaces compiled | Formalize the all-real-zero predicate, de Bruijn forward preservation, threshold existence, threshold closedness, arbitrary-base strip contraction, and the simple-zero dynamics interface for the exact H6-B/H6-H1 family. | The framework reaches the unconditional `t=1/2` good witness and the exact conditional transfer `Im(z)^2<=muSq` at `t` to `Im(z)^2<=muSq-2*delta` at `t+delta`, hence the all-real endpoint after `y^2/2`. It also supplies unique real simple paths, pair-removed dynamics, and the adjacent bound `(gap^2)'<=8`. The general strip theorem is known infrastructure with `hard_gap_delta=0`; implementation `9ddee42657933ccd94533affa25f83a75392a1ea` and evidence `307b5e29ed65b909e8efffb126787b9176c93453` passed public CI runs `29572752471` and `29572973709`. Unconditional canopy certification, global enumeration/continuation, and repeated-zero dynamics remain open. |
