...
| H6-H | open globally; H2a-H2d public complete | Formalize the all-real-zero predicate, de Bruijn forward preservation, threshold existence, and threshold closedness for the exact H6-B/H6-H1 family. | The exact zero coordinate, strict time-zero strip, RH equivalence, closedness, arbitrary-two-time forward preservation, quantitative strip contraction `Im(z)^2 <= 1-2*t`, and unconditional `t=1/2` good witness are public. H6-E/G8 and the remaining global heat-flow zero theory stay open. |
