...
| H6-Q1 | open; selected; equation `(39)` public K0 | Kernel-check the initial, final, and barrier certificates for the second row of Polymath Table 1 and derive `deBruijnNewmanAllZerosReal (1/5)` without hypotheses. | The conditional Polymath consumer, finite-height-RH initial bridge, complete Theorem 1.3 normalization, explicit final-region consumer, equation `(htz)`, fixed principal-power `5*pi/4` line integrability, every positive-integer local residue normalization, the exact adjacent and finite `R_(0,0)` shifts, the full Titchmarsh `(xio)` identity, and their exact composition into finite equation `(39)` are public K0. Loop 8 implementation `af6c80c42c0abdfb1cf91147e74a8b88263b20ea` passed public CI run `29651603027`; separate horizontal Gaussian integrability and reflection transport are checked. The contour-shift formulae `(rtn-def)`/`(RTN-def)`, effective approximation, strict finite-sum certificate, finite RH premise through `3*10^12`, compact barrier winding certificate, H6-E/G8, and RH remain open. External Arb output is navigation evidence only. |
