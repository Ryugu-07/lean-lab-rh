...
| H6-E | open | Prove all zeros of `H_0` are real, equivalently `Lambda <= 0` in the audited normalization. | The generic adjacent-gap attack is exhausted by `OBS-H6-ADJACENT-GAP-EIGHT-01`: its integrated persistence time is only terminal gap squared divided by eight. A new attack must use theta-specific height-aware structure or a different collision invariant. The endpoint is unchanged. |
