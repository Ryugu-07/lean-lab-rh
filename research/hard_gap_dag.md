...
| H6-Q1 | open; selected; Loop 9 preregistered | Kernel-check the initial, final, and barrier certificates for the second row of Polymath Table 1 and derive `deBruijnNewmanAllZerosReal (1/5)` without hypotheses. | The conditional Polymath consumer, finite-height-RH initial bridge, complete Theorem 1.3 normalization, explicit final-region consumer, equation `(htz)`, fixed principal-power `5*pi/4` line integrability, every positive-integer local residue normalization, the exact adjacent and finite `R_(0,0)` shifts, the full Titchmarsh `(xio)` identity, and their exact composition into finite equation `(39)` are public K0. Loop 9 now fixes `(rtn-def)`/`(RTN-def)` as the next actual-source edge. Its first open obligation is a subgaussian bound uniform over the closed interpolation strip, including Gamma control a positive distance from the integer poles and decay of both rectangle ends. Effective estimates, strict finite-sum certificates, the finite RH premise through `3*10^12`, compact barrier winding, H6-E/G8, and RH remain open. External Arb output is navigation evidence only. |
