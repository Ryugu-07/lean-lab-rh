...
| H6-Q1 | open; selected; `(xio)` locally proven | Kernel-check the initial, final, and barrier certificates for the second row of Polymath Table 1 and derive `deBruijnNewmanAllZerosReal (1/5)` without hypotheses. | The conditional Polymath consumer, finite-height-RH initial bridge, complete Theorem 1.3 normalization, explicit final-region consumer, equation `(htz)`, fixed principal-power `5*pi/4` line integrability, every positive-integer local residue normalization, the exact adjacent and finite `R_(0,0)` shifts, and the full Titchmarsh `(2.10.1)--(2.10.6)` to Polymath `(xio)` identity are K0. The next exact gate is their source-aligned composition into finite equation `(39)`. Its effective approximation, strict finite-sum certificate, finite RH premise through `3*10^12`, compact barrier winding certificate, H6-E/G8, and RH remain open. External Arb output is navigation evidence only. |
