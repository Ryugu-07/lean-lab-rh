...
| H6-H | open; H2b complete; H2c selected | Formalize the all-real-zero predicate, de Bruijn forward preservation, threshold existence, and threshold closedness for the exact H6-B/H6-H1 family. | The exact time-zero all-real-zero predicate, bijective zero coordinate, strict strip, RH equivalence, and threshold closedness are public. Campaign `CAMPAIGN-20260717-H6-FORWARD-PRESERVATION-01` fixes exact two-time forward real-zero preservation as the next endpoint. Threshold nonemptiness/upper-ray structure and the remaining global heat-flow zero theory stay open until that endpoint is compiled. |
