...
| H6-H | open; H2b selected | Formalize the all-real-zero predicate, de Bruijn forward preservation, threshold existence, and threshold closedness for the exact H6-B/H6-H1 family. | The exact time-zero all-real-zero predicate, bijective zero coordinate, strict strip, and RH equivalence are publicly compiled. Campaign `CAMPAIGN-20260717-H6-THRESHOLD-CLOSEDNESS-01` fixes exact closedness as the next endpoint. Forward preservation, threshold existence, and the global heat-flow zero theory remain open. |
