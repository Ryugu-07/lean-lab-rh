...
| H6-Q1 | open; selected; Loop 6 finite decomposition compiled locally | Kernel-check the initial, final, and barrier certificates for the second row of Polymath Table 1 and derive `deBruijnNewmanAllZerosReal (1/5)` without hypotheses. | The conditional Polymath consumer, finite-height-RH initial bridge, complete Theorem 1.3 normalization, explicit final-region consumer, equation `(htz)`, fixed principal-power `5*pi/4` line integrability, every positive-integer local residue normalization, and the exact adjacent infinite shift `I_N(s)=(N+1)^(-s)+I_(N+1)(s)` are public K0. Loop 5 constructs the removable pole, finite pullback rectangle, and separate vanishing short sides; implementation `580bc734` and evidence `0c2d932c` passed public CI. Its upper finite interval has a fixed stagger, explicitly recorded in attempts. Loop 6 locally compiles the finite `I_0` summation and exact `R_(0,0)` prefactor transport after preregistration `0e6ce9b4` passed public CI. Titchmarsh `(xio)`, effective approximation, strict finite-sum certificate, finite RH premise through `3*10^12`, and compact barrier winding certificate remain open. External Arb output is navigation evidence only. |
