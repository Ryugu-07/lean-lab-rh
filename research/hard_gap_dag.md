...
| H6-H | open globally; H2a-H2d and first force spine public; local trajectory spine verified locally | Formalize the all-real-zero predicate, de Bruijn forward preservation, threshold existence, threshold closedness, and the simple-zero dynamics interface for the exact H6-B/H6-H1 family. | The public framework reaches the unconditional `t=1/2` good witness. Locally compiled loop 2 adds unique real simple-zero paths, local ordering, an absolutely convergent pair-removed force, and `(gap^2)'=8+4*gap*Re(pairRemainder)`. Global enumeration/continuation and repeated-zero treatment remain open. |
| H6-E | open | Prove all zeros of `H_0` are real, equivalently `Lambda <= 0` in the audited normalization. | This is the direct H6 RH endpoint. The exact local gap law isolates the next attack as a theta-specific integrated bound on the pair-removed remainder over all relevant heights, plus continuation up to the first repeated zero. No such bound is yet proved, so the endpoint is unchanged. |
...
| G8 | H6-E | open | Prove `Lambda <= 0`, equivalently all zeros of the compiled source-normalized `H_0` are real. Local real simple-zero paths and the pair-removed law `(gap^2)'=8+4*gap*Re(pairRemainder)` now compile locally. The missing edge is global, height-aware continuation and theta-specific integrated remainder control through time zero, including the first possible repeated zero. |
