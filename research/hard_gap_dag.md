...
| H6-H | open globally; H2a-H2c public, H2d implementation CI passed | Formalize the all-real-zero predicate, de Bruijn forward preservation, threshold existence, and threshold closedness for the exact H6-B/H6-H1 family. | The exact zero coordinate, strict time-zero strip, RH equivalence, closedness, and arbitrary-two-time forward preservation are public. H6-H2d compiles `Im(z)^2 <= 1-2*t` on `0<=t<=1/2` and the unconditional good-time witness at `t=1/2`; implementation commit `8669c2d` passed public CI and immutable evidence CI remains. H6-E/G8 and the remaining global heat-flow zero theory stay open. |
