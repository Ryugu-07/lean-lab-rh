...
| H6-Q1 | open; selected; Loop 4 proper prefix public; Loop 5 preregistration public | Kernel-check the initial, final, and barrier certificates for the second row of Polymath Table 1 and derive `deBruijnNewmanAllZerosReal (1/5)` without hypotheses. | The conditional Polymath consumer, finite-height-RH initial bridge, complete Theorem 1.3 normalization, explicit final-region consumer, equation `(htz)`, fixed principal-power `5*pi/4` line integrability, and every positive-integer local residue normalization are K0. Loop 4 is publicly checked as `PARTIAL / BLOCKER_EXPOSED` at implementation `7bb3101b` and evidence `0fcfbd51`. Loop 5 fixes the exact adjacent shift `I_N(s)=(N+1)^(-s)+I_(N+1)(s)` through removable pole extension, a finite affine parallelogram, and vanishing end segments; preregistration `25f6f431` passed public CI run `29635161693`. The finite `R_(0,0)` decomposition, Titchmarsh `(xio)`, effective approximation, strict finite-sum certificate, finite RH premise through `3*10^12`, and compact barrier winding certificate remain open. External Arb output is navigation evidence only. |
