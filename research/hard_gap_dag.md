...
| OBS-H6-XI-PF5-01 | H6 physical-kernel total positivity | local complete, public CI pending | The exact full-series Xi kernel has a negative `5x5` Toeplitz determinant at `(u0,h)=(1/100,1/20)`. Lean independently proves rational enclosures for all nine infinite `tsum` entries, an exact LU center determinant, a 120-permutation perturbation bound, and the source-faithful ordered `not PF5` witness. Classification `ACTUAL_KERNEL_PF5_FORMALLY_FALSIFIED`, with `hard_gap_delta=0` and `obstruction_map_delta=1`: PF5/PF-infinity physical-kernel routes are blocked, while global PF4, H6-E/G8, and RH remain open. |
