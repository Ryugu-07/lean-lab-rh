...
| H6-Q1 | open; selected; Loop 3 public implementation verified, evidence pending | Kernel-check the initial, final, and barrier certificates for the second row of Polymath Table 1 and derive `deBruijnNewmanAllZerosReal (1/5)` without hypotheses. | The conditional Polymath consumer, finite-height-RH initial bridge, complete Theorem 1.3 normalization, explicit final-region consumer, and equation `(htz)` including its product-integrability proof are K0. The next source edge is Titchmarsh `(xio)` and the infinite-diagonal `R_(0,N)` contour/residue decomposition. The actual effective approximation, strict finite-sum certificate, finite RH premise through `3*10^12`, and compact barrier winding certificate remain open. External Arb output is navigation evidence only. |
