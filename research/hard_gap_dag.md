...
| H6-Q1 | open; selected; Loop 4 preregistered, CI pending | Kernel-check the initial, final, and barrier certificates for the second row of Polymath Table 1 and derive `deBruijnNewmanAllZerosReal (1/5)` without hypotheses. | The conditional Polymath consumer, finite-height-RH initial bridge, complete Theorem 1.3 normalization, explicit final-region consumer, and equation `(htz)` including its product-integrability proof are K0. Loop 4 fixes the principal-power `5*pi/4` midpoint lines, absolute integrability, removable pole subtraction, affine-parallelogram contour shift, finite `R_(0,0)` residue decomposition, and Titchmarsh `(xio)` as one attempted source edge; no proof source may change until its preregistration CI is green. The actual effective approximation, strict finite-sum certificate, finite RH premise through `3*10^12`, and compact barrier winding certificate remain open. External Arb output is navigation evidence only. |
