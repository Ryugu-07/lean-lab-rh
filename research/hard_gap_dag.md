...
| H6-Q1 | open; selected; Loop 2 publicly checked | Kernel-check the initial, final, and barrier certificates for the second row of Polymath Table 1 and derive `deBruijnNewmanAllZerosReal (1/5)` without hypotheses. | The conditional Polymath consumer, finite-height-RH initial bridge, complete Theorem 1.3 source normalization, displayed error expressions, and explicit final-region certificate consumer are K0. The actual effective approximation, strict finite-sum certificate, finite RH premise through `3*10^12`, and compact barrier winding certificate remain open. The next exact edge is the imaginary Gaussian `H_t`-from-`H_0` representation. External Arb output is navigation evidence only. |
