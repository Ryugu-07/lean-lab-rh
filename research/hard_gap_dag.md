...
| H6-Q1 | open; selected; Loop 4 proper prefix implementation CI passed, evidence CI pending | Kernel-check the initial, final, and barrier certificates for the second row of Polymath Table 1 and derive `deBruijnNewmanAllZerosReal (1/5)` without hypotheses. | The conditional Polymath consumer, finite-height-RH initial bridge, complete Theorem 1.3 normalization, explicit final-region consumer, equation `(htz)`, fixed principal-power `5*pi/4` line integrability, and every positive-integer local residue normalization are K0. Loop 4 is `PARTIAL / BLOCKER_EXPOSED`: removable pole extension, affine-parallelogram contour shift, vanishing ends, finite `R_(0,0)` residue decomposition, and Titchmarsh `(xio)` remain open. Implementation `7bb3101b` passed public CI run `29634900588`. The actual effective approximation, strict finite-sum certificate, finite RH premise through `3*10^12`, and compact barrier winding certificate also remain open. External Arb output is navigation evidence only. |
