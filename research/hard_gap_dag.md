...
| H6-Q1 | open; selected | Kernel-check the initial, final, and barrier certificates for the second row of Polymath Table 1 and derive `deBruijnNewmanAllZerosReal (1/5)` without hypotheses. | The conditional consumer is K0. The remaining source layers are finite RH through `3*10^12`, effective Riemann--Siegel bounds, and a compact barrier winding certificate. External Arb output is navigation evidence only. Campaign `LITERATURE-20260718-H6-POLYMATH-TABLE-ROW-CERTIFICATES-01` is preregistered. |
