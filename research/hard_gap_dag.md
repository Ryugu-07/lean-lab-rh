...
| H6-Q1 | open; selected; Loop 1 local checkpoint | Kernel-check the initial, final, and barrier certificates for the second row of Polymath Table 1 and derive `deBruijnNewmanAllZerosReal (1/5)` without hypotheses. | The conditional consumer and the exact finite-height-RH-to-initial-region bridge are K0. The finite RH premise through `3*10^12`, effective Riemann--Siegel bounds, and compact barrier winding certificate remain open. External Arb output is navigation evidence only. Campaign `LITERATURE-20260718-H6-POLYMATH-TABLE-ROW-CERTIFICATES-01` remains active. |
