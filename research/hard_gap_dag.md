...
| H6-Q1 | open; selected; Loop 3 preregistered, CI pending | Kernel-check the initial, final, and barrier certificates for the second row of Polymath Table 1 and derive `deBruijnNewmanAllZerosReal (1/5)` without hypotheses. | The conditional Polymath consumer, finite-height-RH initial bridge, complete Theorem 1.3 source normalization, displayed error expressions, and explicit final-region certificate consumer are K0. Loop 3 attacks equation `(htz)` through the exact variance-two imaginary Gaussian product integral. The actual effective approximation, strict finite-sum certificate, finite RH premise through `3*10^12`, and compact barrier winding certificate remain open. External Arb output is navigation evidence only. |
