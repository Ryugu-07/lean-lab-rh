...
- Loop 3 is locally complete as `KNOWN_THEOREM_FORMALIZED`. Lean proves the exact complex MGF,
  ...
  implementation CI is pending; Loop 4's next exact gate is the fixed `5*pi/4` infinite contour
  and its finite residue shift, not an assumed contour interface.
