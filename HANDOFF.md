...
- Targets, the exact TargetCheck, and four axiom prints pass; every selected theorem has exactly
  ...
  build job `87717424885`, in `2m47s`. Status is
  `PUBLIC_IMPLEMENTATION_VERIFIED_EVIDENCE_PENDING`: do not publicly close H6-H2c until the
  immutable evidence-backfill commit passes its own public CI. This is
  `KNOWN_THEOREM_FORMALIZED`, with `hard_gap_delta=0` and `route_infrastructure_delta=1`;
  threshold nonemptiness/upper-time existence, H6-E/G8, W2/G7, M2/G3, and RH remain open.
