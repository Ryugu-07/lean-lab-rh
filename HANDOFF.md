...
- This first spine is known source infrastructure only: `hard_gap_delta=0`, local
  `route_infrastructure_delta=1`. The campaign stays active. Next is to construct and order local
  real trajectories from simple real zeros, then isolate the first theta-specific collision
  inequality; repeated zeros, H6-E/G8, and RH remain open. Public implementation CI is pending.
