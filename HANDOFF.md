...
## RH current handoff (2026-07-11, M1-15)

- Forward block F1 is closed in the preregistered RH specialization. Main theorem:
  `RiemannHypothesis.exists_balazardSaias_specialized_bound_compiled` in
  `LeanLab/Riemann/BalazardSaiasContour.lean`.
- `RiemannHypothesis.exists_norm_burnolMobiusTransformedError_le_compiled` connects the result to
  the fixed Burnol consumer without an `hBS : BalazardSaiasEstimate` premise.
- The source reciprocal is represented by `analyticReciprocalRiemannZeta`; this is essential at
  `s=1`, where Mathlib's raw `riemannZeta` has a finite regularized point value.
- M1-15 is `HARD_GAP_REDUCED`: it removes the contour-shifting/error-balancing edge and closes only
  RH-specialized F1. The stronger general-alpha `BalazardSaiasEstimate` remains unproved.
- Active fixed frontier: G2's reverse base Nyman-Beurling/Baez-Duarte criterion and the associated
  half-plane Hardy-space factorization; G1, D, M1 as a complete equivalence, and RH remain open.
- Next loop must preregister a source-accurate reverse-criterion subedge after a bounded API/source
  audit. It must not count additional contour rewrites or the unproved general-alpha proposition as
  necessary forward progress.
