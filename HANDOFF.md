...
- Current local closure checks pass: Targets, exact TargetChecks, AxiomsAudit, forbidden scans,
  `git diff --check`, Markdown links, and the full 8,682-job build.
...
- Post-census route selection chooses H6-B as
  ...
  build job `87606471329`, in `1m55s`; implementation is active.
- The dependency audit found that `completedRiemannZeta₀` is already definitionally backed by
  `mellin (hurwitzEvenFEPair 0).f_modif`. The remaining H6-B gap is the explicit conversion from
  that self-dual theta Mellin integral to the source `Phi` cosine transform: expand `f_modif`, use
  the exact theta series, substitute `x=exp(4*u)`, integrate by parts, and evenize. Definitions or
  convergence helpers alone do not close the campaign, and all route/progress deltas remain zero.
