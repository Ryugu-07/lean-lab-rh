...
## RH current handoff (2026-07-11, M1-08)

- Active fixed gap: `G2/F1/zeta-convexity`.
- New module: `LeanLab/Riemann/ZetaConvexity.lean`.
- Compiled boundary chain: `zeta(1+it)=O((1+|t|)^(1/8))`, exact Gamma/cosine cancellation,
  `zeta(it)=O((1+|t|)^(5/8))`, and pole-removed powers `9/8` and `13/8`.
- The preregistered critical-line exponent `3/8` is not yet proved. Do not mark F1 reduced.
- Next exact task: formalize Fiori's corrected midpoint symmetrization for integer powers
  `(13,9)` and prove the weighted quotient satisfies `PhragmenLindelof.vertical_strip` growth.
- Do not use the invalid Trudgian auxiliary function depending on `re(s)`, and do not insert an
  axiom for the weighted strip theorem or interior growth.
