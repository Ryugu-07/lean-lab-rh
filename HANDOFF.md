...
## RH current handoff (2026-07-11, M1-10)

- Active fixed gap: `G2/F1/Balazard-Saias`.
- New module: `LeanLab/Riemann/BalazardSaias.lean`.
- The exact source estimate is encoded as `BalazardSaiasEstimate`, a proposition with no proof and
  no axiom status. All consumers retain it as an explicit argument.
- Lean now proves the complete Burnol consequence: the `3/8` zeta bound yields quotient decay
  `-5/8`, the admissible Balazard-Saias height margin is `eta < 1/8`, and
  `N^(-delta/3) -> 0` for `delta > 0`.
- M1-10 is `FORMALIZATION_ONLY` with `hard_gap_delta = 0`.
- Next exact task: obtain or reconstruct the proof of Balazard-Saias Lemma 2 from audited sources,
  decompose its Perron/Abel/reciprocal-zeta inputs, and compile a source-equivalent sufficient
  specialization under RH. The `Re(s)>1` absolute-convergence theorem is not an admissible substitute.
- F1 itself remains open until Balazard-Saias is compiled. The reverse base criterion, G1, D, and
  RH also remain open.
