...
## RH current handoff (2026-07-11, M1-16)

- The reverse implication of the exact M0-aligned strong Baez-Duarte carrier is compiled:
  `baezDuarteComplexTarget_mem_closure_imp_riemannHypothesis` in
  `LeanLab/Riemann/BaezDuarteReverse.lean`.
- The proof directly formalizes the Mellin zero obstruction on source-structured full-line errors.
  It does not assume or claim the general Nyman-Beurling criterion or a Hardy-space theorem.
- M1-16 is `HARD_GAP_REDUCED`: remove `G2/reverse/base-criterion` for this aligned carrier.
- Active fixed frontier: assemble the already compiled F1/F2/F3 quantitative components into the
  actual forward theorem `Mathlib.RiemannHypothesis -> baezDuarteComplexTargetL2 ∈
  baezDuarteComplexKernelClosure`. Existing separate bounds are not yet this closure theorem.
- The stronger general-alpha `BalazardSaiasEstimate`, G1, D, M1 as a full equivalence, and RH itself
  remain open.
- The next loop must audit the exact missing forward dominated-convergence/Plancherel assembly and
  preregister one source-facing convergence theorem; it must not return to closed contour or reverse
  subedges for standalone local lemmas.
...
- 全项目 `lake build` 8608 jobs 通过；无 `sorry`/`admit`/`sorryAx`、无项目 `axiom`/`constant`。本批分类 `FORMALIZATION_ONLY`，不改变 RH、M2 或 G3。
