...
- Active preregistration is `LITERATURE-20260717-H6-POLYMATH-ZERO-FREE-CRITERION-01`. It targets
  the complete three-region Polymath criterion, its exact `t0+y0^2/2` all-real consequence, and
  the Table 1 second-row corollary at `1/5`. The first missing source theorem is repeated-zero
  backward Hermite splitting at first canopy contact; simple complex paths and the global force
  geometry must also be completed. No Lean proof source may be edited until preregistration CI
  passes. The three region certificates, unconditional `Lambda<=0.2`, H6-E/G8, and RH remain open.
...
- Loop 5 已编译 negation/conjugation 除子等价、force 四轨道绝对收敛重排、非接触轨道的
  ...
  fiber 基数从 `tsum` 同时剥离四个两两不同的接触索引，并证明余项总和非正。
- 尚未证明 Polymath Proposition 3.3、三个区域证书、`Lambda<=0.2` 或 RH。两个精确源障碍是：
  简单接触处正则化除子力的严格虚部不等式；重零点向后 Hermite 分裂推出
  `deBruijnNewmanHasBackwardUpperLinearEscape`。
- 下一步优先按 negation/conjugation 四元轨道平均绝对收敛除子和，证明力不等式；完整循环记录见
  `attempts/h6_polymath_zero_free_criterion.md`。
