...
- Post-census route selection chooses H6-B as
  ...
  `H_0(z)=(1/8)*riemannXi((1+i*z)/2)`. Local inventory found no existing xi theta-integral
  theorem, so the fixed failure boundary is an explicit theta-Mellin dependency gap; definitions
  or convergence helpers alone cannot close the campaign. Lean proof edits await preregistration
  publication and CI.
