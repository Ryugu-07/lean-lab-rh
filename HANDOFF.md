...
- Active preregistration is `CAMPAIGN-20260717-H6-GENERAL-STRIP-CONTRACTION-01` (`LITERATURE`).
  ...
  `87857388857`, before proof edits. Both exact endpoints now compile locally; implementation CI
  and immutable evidence backfill remain before campaign closure.
...
- 本地分类 `KNOWN_THEOREM_FORMALIZED`，`hard_gap_delta=0`，
  ...
  `Lambda<=0.2`、严格改进、H6-E/G8 或 RH。下一门是实现提交与公开 CI，随后回填不可变
  证据并公开闭合；全局 RH Goal 始终 active。
