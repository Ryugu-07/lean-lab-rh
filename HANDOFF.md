...
## RH current handoff (2026-07-16, R5 Gaussian prime-kernel sign audit admitted)
...
- The fixed endpoint uses the actual `n=2` symmetric Gaussian von-Mangoldt weight and proves that
  one explicit two-shift kernel matrix is neither positive nor negative semidefinite.
- The witness width is `(log 2)^2/16`, with shifts `0` and `log 2`; all arithmetic-weight rewrites
  and strict inequalities must compile in Lean before the conclusion is retained.
- Success only eliminates termwise semidefinite local-prime assembly as an unconditional G7
  mechanism. Complete local cancellation, generic W1c1, W2/G7, and RH remain open, with expected
  `hard_gap_delta=0`.
- Begin Lean Falsification Attempt A. Do not weaken the endpoint to a normalized surrogate kernel.
