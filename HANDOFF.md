...
## RH current handoff (2026-07-13, G4-F2)

- M0, M1, G1, G2, and D remain complete; M2/G3 remains parked. No unconditional proof of RH or
  closure membership is claimed.
- Burnol node B1 has F0-F2 complete. `LeanLab/Riemann/BurnolY.lean` constructs the total second
  phase and boundary vectors, proves their exact critical-line `L2` limit, F3-ready transformed
  estimates, direct source pairing, and analytic-order orthogonality to the full model span.
- Batch `BATCH-20260713-G4-F2` is `KNOWN_THEOREM_FORMALIZED`. Implementation commit `21f600b6`
  passed public Lean Action CI run `29278978470`, build job `86915163555`.
- The next fixed frontier is G4/F3 only: preregister the finite-dimensional Gram and target-pairing
  asymptotics, including the relevant Hilbert/Cauchy inverse entry. Do not begin F4/F5 and do not
  reopen F2 for standalone local lemmas.
