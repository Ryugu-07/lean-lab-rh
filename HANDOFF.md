...
## RH current handoff (2026-07-14, G4-F3 local verification)

- M0, M1, G1, G2, and D remain complete; M2/G3 remains parked. No unconditional proof of RH or
  closure membership is claimed.
- Burnol node B1 has F0-F2 publicly complete. The preregistered F3 implementation in
  `LeanLab/Riemann/BurnolGram.lean` is locally complete: actual normalized Gram limits, the
...
- F3 remains selected only because its implementation commit and public Lean Action CI are still
  pending. Do not begin F4/F5 until that public gate passes; do not reopen F2/F3 for standalone
  helper lemmas.
