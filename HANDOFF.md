...
- Loop 9 of `LITERATURE-20260718-H6-POLYMATH-TABLE-ROW-CERTIFICATES-01` is preregistered as a
  `PROOF-ATTEMPT` on the actual source contour shifts `(rtn-def)` and `(RTN-def)`. It fixes arbitrary
  complex shifts, the strict same-half-plane source domain, and the exact variance-two factors. The
  first obstacle is a closed-strip-uniform subgaussian bound strong enough to kill both finite
  rectangle ends; pointwise horizontal integrability is insufficient.
