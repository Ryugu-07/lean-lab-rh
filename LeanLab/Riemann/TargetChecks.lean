...
example {x : ℝ} (hx : 0 < x) :
    MeasureTheory.IntegrableOn
      (deBruijnNewmanPolymathBoydSaddleIntegrand x) (Set.Ioi 0) :=
  deBruijnNewmanPolymathBoydSaddleIntegrand_integrableOn hx
...
example : deBruijnNewmanPolymathBoydBoundaryTraceNearZeroCertificateStatement :=
...
example (w : ℂ) :
  ...
  deBruijnNewmanPolymath_self_mul_scaledGamma_eq_zeroFactor w
...
example (w : ℂ) :
  ...
  deBruijnNewmanPolymath_inv_scaledGamma_eq_zeroFactor w

example : ContinuousAt
...
example {z : ℂ} (hz : 0 < z.re) {delta : ℝ} (hdelta : 0 < delta) :
    TendstoUniformlyOn
      (fun epsilon y =>
        deBruijnNewmanPolymathBoydShiftedBoundaryPairIntegrand z epsilon y)
      (deBruijnNewmanPolymathBoydBoundaryAxisPairIntegrand z)
...
example :
    (∫ x in Set.Ioi (1 / 2 : ℝ),
      ‖deBruijnNewmanRiemannSiegelLineIntegrand 1
        (heightTenRiemannSiegelCriticalPoint 10) (-x)‖) ≤ 19 / 2000 :=
...
example : 0 < hardyXi 10 :=
  hardyXi_ten_pos

example : hardyXi 17 < 0 :=
  hardyXi_seventeen_neg

example : HardyXiBracketsZero 10 17 :=
  hardyXiBracketsZero_ten_seventeen

example {y : ℝ} (hy0 : 13 / 2 ≤ y) (hy1 : y ≤ 7) :
    levinsonMontgomeryLogDerivArchimedeanTerm ((y : ℂ) * Complex.I) < 9 / 2000 :=
  levinsonMontgomeryArchimedean_imaginaryAxis_lt_nine_div_twoThousand hy0 hy1

example {y : ℝ} (hy0 : 13 / 2 ≤ y) (hy1 : y ≤ 7) :
    (speiserZetaDerivRatio ((y : ℂ) * Complex.I)).re < 0 :=
  speiserZetaDerivRatio_leftVertical_re_neg_thirteenHalves_seven_lowZeroMass hy0 hy1
