...
def rhTargets : List ResearchTarget :=
  [ { id := "T1.m1.baez.duarte.criterion"
    ...
    { id := "T2.burnol.distance.lower-bound"
      ...
      leanName := none
      status := .inProgress
      note := "Admitted by the independent Arch M1 audit of 2026-07-13 as the post-M1 research line. Audit G4-01 fixes source dependencies F0-F5; Batch G4-F0 Lean-checks the continuous/natural transfer. Audit G4-F1-01 recovers the exact unitary model, signs, and dilation normalization, splitting F1 into explicit Burnol A proposition F1a and distance assembly F1b. F1a is next. This is known mathematics and a quantitative obstruction, not M2/G3 progress." },
    { id := "T3.rh.horizon"
      tier := .tier3
