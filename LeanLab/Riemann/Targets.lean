...
def rhTargets : List ResearchTarget :=
  [ { id := "T1.r1.baez.duarte.qtwo.criterion"
    ...
    { id := "H6.debruijn-newman.backward-hermite-model-root"
      tier := .tier2
      title := "Prove the backward Hermite splitting model has an upper root"
      statement :=
        "For every multiplicity n at least two, prove that the monic backward-heat Hermite polynomial generated by P_(n+1)=X*P_n+2*P_n' has a complex zero with strictly positive imaginary part."
    ...
    { id := "H6.debruijn-newman.polymath-riemann-siegel-normalization"
      tier := .tier2
      title := "Align the Polymath effective Riemann--Siegel normalization"
      statement :=
    ...
    { id := "H6.debruijn-newman.boyd-r2-equation-15"
      ...
      status := .inProgress
      note := "Loop 25 proves the actual R2 is exactly the normalized Gaussian integral of the real inverse-Jacobian remainder and reduces equation (15) to one positive-real contour equality. Loop 27 gives a second direct reduction: the reflection product rewrites both exponential Gamma rays as a boundary jump, the jump splits into R2 and reflected inverse-R2 on opposite half-planes, and finite Cauchy rectangles reduce the equation to two explicit outer-edge decay limits plus one inner boundary-trace limit. Loop 28 proves the source jump truncations converge, rewrites the finite inner projection as one paired offset-line integral, proves pointwise convergence at every nonzero boundary point, and reduces the inner trace exactly to one named discrepancy limit. That discrepancy limit and both outer-edge limits remain unproved. Equation (15), effective R2, Table 1, H6-E/G8, and RH remain open." },
    ...
    { id := "H6.audit.xi-kernel-logconcavity-external-lean"
      tier := .tier2
      title := "Audit the external Xi-kernel log-concavity Lean certificate"
      statement :=
    ...
    { id := "H10.function-field.bombieri-stepanov-polar-injectivity"
      tier := .tier2
      title := "Audit the Bombieri-Stepanov nonzero-production gate"
      statement :=
        "Prove that finite-dimensional descent-kernel production plus injective realization yields a nonzero auxiliary, model polar noncancellation by a separated polynomial coefficient-block equivalence, and certify that dimension surplus alone fails for a noninjective realization."
    ...
    { id := "H14.riemann-siegel.height-ten-prefactor-phase-margin"
      ...
      status := .proven
      note := "Subattack HEIGHT-TEN-RIEMANN-SIEGEL-PHASE-NORM-01 closes the prefactor producer without a Gamma approximation premise. Lean proves exact sign-sensitive endpoint domination for the contour complex power, a rectangular Stieltjes majorant giving norm of the logarithmic Gamma remainder at most 1/16, an explicit shifted Stirling phase strictly between -3/8 and 1/8 on the full interval, and an exact factorization of the actual prefactor as a positive scale times the phase exponential. The resulting real-part-to-norm ratio is strictly greater than 9/10. The downstream endpoint-integration target now composes this producer to the literal Riemann--Siegel remainder margin and right-high interval nonvanishing. The other height-ten boundary zones, complete certificate, H12, and RH remain open." },
    ...
    { id := "H14.riemann-siegel.height-ten-endpoint-mass-compact-integration"
      ...
      status := .proven
      note := "The compact endpoint subattack compiles a concave-tangent range certificate for the positive cubic exponent, a fourth-power Pade polynomial upper bound, a fourth-power quadratic bound for the negative exponential, and rational quartic denominator transfer. Formal polynomial primitives prove the actual positive compact mass at most 7/20 and the actual negative compact mass at most 1/10, hence compact total at most 9/20. The downstream tail-integration target supplies the complementary 3/20 and closes the full endpoint mass. This target alone remains a compact-integration component, not a height-ten certificate or RH theorem." },
    { id := "H14.riemann-siegel.height-ten-endpoint-tail-partial"
      ...
      status := .proven
      note := "This preserved partial checkpoint proves the negative tail at most 19/2000 and positive near tail at most 27/200 using retained phase and a kernel-checked convex trapezoid theorem. The subsequent complete tail target proves the formerly open positive far tail at most 11/2000 and consumes this checkpoint. It remains listed separately so the historical proof decomposition is auditable." },
    { id := "H14.riemann-siegel.height-ten-endpoint-mass-right-high"
      tier := .tier1
      title := "Close the endpoint mass and right-high critical-line zone"
      statement :=
        "Prove the actual positive far tail at most 11/2000, combine all compact and tail integrals to the cutoff-one endpoint mass at most 3/5, discharge the literal Riemann--Siegel remainder margin, and derive actual zeta nonvanishing and strict-negative Speiser quotient real part for 13/2<=y<=10."
