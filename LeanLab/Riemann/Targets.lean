...
def rhTargets : List ResearchTarget :=
  [ { id := "T1.r1.baez.duarte.qtwo.criterion"
    ...
    { id := "T2.weil.compact-c6-arithmetic-formula"
      ...
      status := .proven
      note := "Campaign CAMPAIGN-20260716-R5-COMPACT-C6-EXPLICIT-FORMULA-01 removes the C-infinity Schwartz wrapper from the compact formula. General Fourier inversion is justified by inverse-square Fourier decay, the first absolute moment follows from inverse-sixth decay, and the selected xi top edge uses exactly six continuous derivatives. Independent module checks, exact target checks, standard-only axiom audits, forbidden scans, and the full 8,681-job build pass locally. Preregistration 540b0ddcbf90a219084f8fdcb80a02ddaad5e277 and implementation 3e3c677495c592096d7843aa4845e861bc393937 passed public CI runs 29467845311 and 29468797210. This closes only the compact finite-regularity W1 subedge; quotient/completeness, full-class regularization, W2/G7 positivity, and RH remain open. Immutable evidence backfill is pending." },
